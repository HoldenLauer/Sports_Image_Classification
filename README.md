...
We used PostgreSQL to store our data. We wanted to keep our data in a database to easily join tables and access them from our machine learning model. Our model accesses the training and testing images directly from our SQL database. 
...
For our machine learning model, our data did not require preprocessing. Within our machine learning document, we were determined to display a sample of images from our training and testing datasets. In order to display sample images, we plotted a figure using a for loop that pulled 9 random photos from the dataset and display the image and the label of the sport displayed in the image. 
...
One thing we would change about our project is making our dashboard more interactive. Due to time constraints, we added only one interactive element. However, if we were going to do something different, we would make it possible for people to adjust our sports vs. cost table to filter sports based on their cost. This would insure parents could see only the sports that are within their budget.
<div id='id-RFA'/>

<ins>Recommendations for Future Analysis</ins>

For the future, we recommend creating a model that would show how accurately it is able to predict specific categories of sports. For example, it would be interesting to know if our model is able to predict "Ball Sports" with a higher accuracy than "Water Sports." We hypothesize that the model would be better able to predict sports with more identifiable equipment. Additionally, it would be interesting to see the results of categorical modeling.
